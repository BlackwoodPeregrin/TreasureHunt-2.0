...
// движения происходят нажимая на клавиши W D S A, этот enum для русской раскладки
enum My_Key {kUp = 1062, kRight = 1042, kDown = 1067, kLeft = 1060};

class ViewPlayingField : public QWidget {
  Q_OBJECT
 public:
  explicit ViewPlayingField(int level_game, QWidget *parent = nullptr)
      : QWidget{parent},
        m_click_counter{0},
        m_current_id_button{0},
        m_cell_in_focus{false},
        ui{new Ui_PlayingField(QGuiApplication::primaryScreen()->geometry().width(),
    ...
    this->setWindowTitle("Treasure Hunt 2");
...
signals:
 ...
 protected:
  ...
  auto installStylePlayingFieldInProcessGame_(int level_game) -> void {
  ...
  auto keyPressEvent(QKeyEvent *event) -> void override {
    int key = event->key();
    if (key == Qt::Key_W || key == My_Key::kUp || key == Qt::Key_Up) {
      stepUp_();
    } else if (key == Qt::Key_D || key == My_Key::kRight || key == Qt::Key_Right) {
      stepRight_();
    } else if (key == Qt::Key_S || key == My_Key::kDown || key == Qt::Key_Down) {
      stepDown_();
    } else if (key == Qt::Key_A || key == My_Key::kLeft || key == Qt::Key_Left) {
      stepLeft_();
