...
class GameMechanics {
 public:
  ...
  auto GenerateField(int size_field) -> void {
    ...
    std::vector<std::pair<int, int>> possible_colors(colors.size());
 ...
 protected:
  ...
  auto GeneratePlayingField_(
      std::vector<std::pair<int, int>> &possible_colors) -> void {
  ...
  auto GenerateColorChip_(
      std::vector<std::pair<int, int>> &possible_colors) -> int {
    //
